...
export function RandomInsightCard({ insights }: { insights: InsightEntry[] }) {
  ...
  return <div aria-live="polite" aria-atomic="true"><mark className="bg-accent/10 text-ink">{selection.entry.insightText}</mark><p className="mt-3 text-xs text-muted">{selection.entry.noteTitle}</p><p className="mt-1 text-xs text-muted">{insights.length} insights available</p><button type="button" onClick={chooseAnother} className="mt-4 cursor-pointer rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent/90">Show another insight</button></div>;
}
