...
test("collects nested allowed notes and excludes Manus and Hedy transcripts", async () => {
  ...
  const transcriptNote = join(vault, "Hedy-AI", "transcript 2026-05-19.md");

  await Promise.all([nestedNote, readDoneNote, manusNote, sourceNote, transcriptNote].map(async (file) => {
    await mkdir(join(file, ".."), { recursive: true });
    await writeFile(file, "# Test\n==insight==");
  }));

  const files = await collectInsightSourceFiles(vault);
  expect(files).toEqual(expect.arrayContaining([nestedNote, readDoneNote]));
...
test("identifies only the explicitly excluded source paths", () => {
  ...
  expect(shouldExcludeInsightSourcePath("/vault/Hedy-AI/transcript 2026-05-19.md")).toBe(true);
